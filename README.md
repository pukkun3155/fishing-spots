...
さらにその上に「⚡ クイック朝チェック」が表示され、風・波・雨・雷・潮汐・最近の釣果・本人実績・タックル・持ち物の不足を1画面に要約し、出発直前に短時間で判断できるようにしています。判定ロジックは朝チェックと共通の関数を使っており、二重管理はしていません。「詳細を見る」で通常の朝チェックを展開できます。

## 釣行判断分析について

「📊 分析」タブに「🧭 判断分析」を追加しました。既存の「📊 釣果分析」（Phase3、無改変）とは別に、「自分がどんな条件で釣行し、どんな条件で見送り、どんな条件で釣果が出ているか」をサンプル数付きで確認できます。

最重要の方針として、判断（釣行した／見送った）と釣果（実際に釣れたか）を必ず別々に集計しており、**見送りを釣果0回として扱いません**。風速・波高は既存の区分（風速は`windSpeedLabel`、波高は`waveHeightToLabel`）をそのまま再利用し、Phase13独自の新しい予測モデルは作っていません。降水量や雷の条件は釣行記録（trip）に保存されていないため、無理に推測せず「不明」または見送り理由の集計のみで扱っています。

1〜2件のデータは「かなり少ない」、3〜4件は「まだ参考程度」と明示し、個人傾向のサマリー文はサンプル数が3件以上ある項目のみ生成します。「釣れる確率」「勝率」といった表現は使わず、「n回中m回」という事実表示に統一しています。集計は既存の`fishing_trips_v1`・`fishing_catches_v1`・`fishing_skip_logs_v1`から都度計算しており、新しいlocalStorageキーは追加していません。

## 魚から探すについて

「🎣 今日」タブに「🎯 魚から探す」を追加しました。魚種名を入力（または一覧から選択）すると、①おすすめのエリア・ポイント、②最近の釣果情報、③推奨仕掛け・タックル適合、④本人実績、⑤今日の条件との相性、をまとめて確認できます。あくまで既存データの整理・提示であり、「釣れる確率」を予測するものではありません。

魚種名は完全一致か、安全な別名（例：キス→シロギス）のみ自動で解決します。「タイ」のように複数の魚種を指しうる俗称は自動判定せず、候補（マダイ／クロダイ）から選んでもらいます。fishMasterに未登録の魚種でも、釣行記録・釣果情報があれば表示しますが、仕掛け情報は「未登録」と明示し、推測では補いません。

タックル適合は既存の手持ちタックル（竿の対応ルアー重量）との比較のみで判定し、風速7m/s以上・波高1.0m以上・突風10m/s以上・雷の可能性がある場合は他の材料より安全情報を優先します。候補ポイントの「📅 この場所で計画する」から、そのまま「📅 候補日時を比較」（Phase10）へ進めます。既に今日の計画がある場合は上書き前に確認します。新しいlocalStorageキーは追加していません。

## 最新釣果の更新支援について

「最近の周辺釣果を見る」を開くと、上部に「📰 最新釣果の更新支援」パネルが表示されます。`latest_reports.json`の件数を鮮度4区分（7日以内／8〜14日／15〜30日／31日超、岸釣り・船釣りは分けて集計）で示し、「詳しく見る」で`report_sources.json`（チェック先の情報源一覧）と実際の掲載日を突き合わせ、「30日以内に掲載記録がない、またはまだ一度も掲載記録がない情報源」を確認候補として一覧表示します。

このパネルは既存の`latest_reports.json`・`report_sources.json`の内容を突き合わせて表示するだけで、情報源サイトへの自動アクセス・自動収集は一切行いません。「何を更新確認すべきか」の判断材料を示すだけで、更新作業自体は引き続き`tools/update_latest_reports.py`（上記チェックリスト参照）で行います。`report_sources.json`が取得できない場合も、魚検索など他の機能には影響しません。新しいlocalStorageキーは追加していません。
