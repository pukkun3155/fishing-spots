...
前日までに候補を採用しておくと、当日「🎣 今日」タブの最上部に「🌅 朝チェック」カードが表示されます（採用済みプランの予定日が今日で、開始時刻・ポイント・狙い魚が分かっている場合のみ）。ページを開いただけでは通信せず、「🔄 最新条件を確認」を押したときだけOpen-Meteo/気象庁のデータを取得します。前回確認した値と最新値の差（風速・突風・波高・降水）を表示し、風速7m/s以上・波高1.0m以上・突風10m/s以上・雷の可能性があれば、差分の改善有無に関わらず「⚠ 条件注意」を優先します。最後に「予定通り行く／時間をずらす／別候補を見る／今日は見送る」を選べます。「時間をずらす」「別候補を見る」は候補比較画面をそのまま再利用します。
